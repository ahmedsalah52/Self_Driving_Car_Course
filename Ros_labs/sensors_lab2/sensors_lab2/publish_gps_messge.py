...
class my_node (Node):
    def __init__(self):
        ...
        self.fix_pub= self.create_publisher(NavSatFix,"fix",rclpy.qos.qos_profile_sensor_data) 
    ...
    def timer_call(self):
        ...
        hdop =        float(row[8])
        lat_std_dev = float(row[18])
        lon_std_dev = float(row[19])
        alt_std_dev = float(row[20])
